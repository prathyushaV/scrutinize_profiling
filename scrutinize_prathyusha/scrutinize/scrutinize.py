...
def load_plugins(config):
    ...
    if config:
        for name, info in config.iteritems():
            ...
            #plugin_position = info.get('position')
            #plugin_prefix = info.get('prefix')
            #plugin_config = {'position': plugin_position, 'prefix': plugin_prefix }
...
def scrutinize(filename):
    ...
    git_repo = config.get('git_repo') if 'git_repo' in config else ''

    if git_repo:
        diff_data = code_coverage.generate_diff(git_repo)
        code_coverage.add_coverage(diff_data)          
        del config['git_repo']
...
def remove_coverage(config_path):
    with open(config_path) as f:
        config = json.load(f)
    if 'git_repo' in config:
        git_repo = config.get('git_repo')
        code_coverage.remove_coverage(git_repo)
        diff_data = code_coverage.generate_diff(git_repo)
        matched_data = code_coverage.analyse_log('/opt/stack/new/scrutinize/code_coverage', diff_data, {})
	code_coverage.render_html(git_repo, '/opt/stack/new/scrutinize/graphs/templates/index.html', matched_data)
